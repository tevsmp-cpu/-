...
const MetricCard = ({ metric, onClick }: { metric: Metric; onClick: () => void; key?: string }) => {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      aria-label={`${metric.name}: ${typeof metric.value === 'number' ? metric.value.toLocaleString('ru-RU') : metric.value} ${metric.unit}. Категория: ${metric.category}.`}
      className={cn(
        "group relative p-4 rounded-xl cursor-pointer border transition-all duration-200 text-left w-full h-full outline-none focus-visible:ring-2 focus-visible:ring-red-500",
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex border border-gray-200 rounded-lg overflow-hidden h-9">
              <button 
                ...
                className={cn(
                  "px-3 flex items-center gap-2 text-xs font-semibold transition-colors outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:z-10",
              ...
              <button 
                ...
                className={cn(
                  "px-3 border-l border-gray-200 flex items-center gap-2 text-xs font-semibold transition-colors outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:z-10",
            ...
            <div className="grid grid-cols-5 gap-0.5 bg-gray-100 p-0.5 rounded-lg max-w-xl">
              {categories.map((cat) => (
                <button
                  ...
                  className={cn(
                    "px-2 py-1.5 text-[9px] font-bold rounded-md transition-all whitespace-nowrap text-center outline-none focus-visible:ring-2 focus-visible:ring-red-500",
