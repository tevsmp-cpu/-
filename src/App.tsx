...
const MetricCard = ({ metric, onClick }: { metric: Metric; onClick: () => void; key?: string }) => {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "group relative p-4 rounded-xl cursor-pointer border transition-all duration-200 text-left outline-none focus-visible:ring-2 focus-visible:ring-red-500",
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {/* View Toggle */}
            <div className="flex border border-gray-200 rounded-lg h-9">
              <button 
                onClick={() => setViewMode('dashboard')}
                aria-pressed={viewMode === 'dashboard'}
                className={cn(
                  "px-3 flex items-center gap-2 text-xs font-semibold transition-colors rounded-l-lg outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:z-10",
              ...
              <button 
                ...
                className={cn(
                  "px-3 border-l border-gray-200 flex items-center gap-2 text-xs font-semibold transition-colors rounded-r-lg outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:z-10",
            ...
            <div className="flex flex-wrap gap-0.5 bg-gray-100 p-0.5 rounded-lg max-w-xl">
              {categories.map((cat) => (
                <button
                  ...
                  className={cn(
                    "px-2 py-1.5 text-[9px] font-bold rounded-md transition-all whitespace-nowrap text-center outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:z-10",
                        ...
                        {selectedMetric.childMetrics.map((childId) => {
                          ...
                          return (
                            <motion.button
                              ...
                              whileTap={{ scale: 0.98 }}
                              className="w-full text-left group flex items-center justify-between p-3 rounded-xl border border-gray-100 hover:border-red-200 hover:bg-red-50 transition-all cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-red-500"
                              aria-label={`Просмотреть декомпозицию: ${child.name}`}
                            >
